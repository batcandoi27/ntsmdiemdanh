...
export async function processImportedAttendance(records: ImportAttendanceRecord[]) {
    try {
        ...
        if (!authUser) throw new Error('Unauthorized: Vui lòng đăng nhập.');
        const appUser = await getAppUser(authUser.id, authUser.email);
        if (!appUser) throw new Error('Unauthorized: Không tìm thấy hồ sơ người dùng.');
        
        console.log('[AUTH]', { userId: authUser.id, role: appUser.role, systemMode: SYSTEM_MODE });
        
        if (!checkPermission(appUser, 'EDIT_ATTENDANCE')) {
            throw new Error('Access Denied: Bạn không có quyền thao tác dữ liệu điểm danh.');
        }
