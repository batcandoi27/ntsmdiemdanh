...
import { Settings, Database, Trash2, Save, CheckCircle, AlertTriangle, RefreshCw } from 'lucide-react';
...
export default function SettingsPage() {
    ...
    const handleGenerate = () => {
        ...
        startTransition(async () => {
            ...
            const endDate = startDate; // Mock for today
            const res = await generateMockData(startDate, endDate, []); // [] = all classes
            if (res.success) {
    ...
    const handleClear = () => {
        ...
        startTransition(async () => {
            const res = await clearAttendance(undefined, undefined, undefined); // Clear all
            if (res.success) {
                setMessage({ type: 'success', text: res.message });
                router.refresh();
            } else {
        ...
        <PasswordGuard>
            <div className="p-6 md:p-8 min-h-screen bg-gray-50/50 space-y-8">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    ...
                    {/* System Info Card (ReadOnly) */}
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col h-full opacity-60">
                        <div className="flex items-center gap-3 mb-4">
                        ...
                        <p className="text-gray-500 text-sm mb-6">
                            Các cài đặt về năm học, học kỳ và thông tin trường học. Hiện tại được cấu hình trong file env.
                        </p>
                        ...
                        <div className="space-y-4 text-sm">
                            ...
                            <div className="flex justify-between py-2 border-b border-gray-50">
                                ...
                                <span className="font-medium">v1.0.0 (Beta)</span>
                            </div>
                        </div>
                    </div>
                </div>
