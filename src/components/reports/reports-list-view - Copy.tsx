                                ...
                                {sortedSubKeys.map(subKey => {
                                    ...
                                    const consolidatedStudents = Object.entries(consolidatedMap)
                                        .map(([code, info]) => {
                                            ...
                                            const allStatuses = info.status.split(', ');
                                            // Filter only those in visibleColumns
                                            const filteredStatuses = allStatuses.filter(st => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <span className="font-medium group relative">
                                                                ...
                                                                {student.notes && (
                                                                    <div className="absolute bottom-full left-0 mb-2 hidden group-hover:block z-[100] pointer-events-none">
                                                                        <div className="bg-gray-900 text-white text-[11px] px-2 py-1.5 rounded-lg shadow-2xl whitespace-nowrap border border-gray-700 font-bold max-w-xs break-words whitespace-normal">
                                                                            {student.notes}
                                                                        </div>
                                                                        <div className="w-2 h-2 bg-gray-900 rotate-45 ml-4 -mt-1 border-r border-b border-gray-700"></div>
                                                                    </div>
                                                                )}
                                                            </span>
...
function CompactStatusBadge({ status }: { status: string }) {
    ...
    const map = {
        'P': { text: 'P', color: 'text-yellow-600', size: 'text-xs' },
        'K': { text: 'K', color: 'text-red-600', size: 'text-xs' },
        'V': { text: 'V', color: 'text-gray-600', size: 'text-xs' },
        'T': { text: 'T', color: 'text-blue-600', size: 'text-[15px] font-black' }, // T cực to và đậm
        'VP': { text: 'VP', color: 'text-purple-600', size: 'text-xs' },
        'KH': { text: 'KH', color: 'text-pink-600', size: 'text-xs' },
    };
