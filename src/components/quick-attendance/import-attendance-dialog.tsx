...
export function ImportAttendanceDialog({ open, onOpenChange, onSuccess }: ImportAttendanceDialogProps) {
    ...
    const handleSave = async () => {
        ...
        setStep("saving");
        console.group("🚀 AI Import Attendance Flow");
        console.log("1. Raw Processed Data:", processedData);

        try {
            const recordsToImport: ImportAttendanceRecord[] = [];

            processedData.forEach(pr => {
                const session: SessionType = pr.session;

                pr.classes.forEach(c => {
                    const studentsToUpdate = c.matchedStudents
                        .filter(s => s.isMatched && s.studentId)
                        .map(s => ({
                    ...
                    if (studentsToUpdate.length > 0) {
                        console.log(`📌 Lớp ${c.className} (${session}): Sắp lưu ${studentsToUpdate.length} học sinh vi phạm.`, studentsToUpdate);
                        ...
                        console.log(`✅ Lớp ${c.className} (${session}): Đi học đầy đủ 100%. Sẽ xoá các dữ liệu vắng cũ.`);
                        recordsToImport.push({
                            date: pr.date,
                            session,
                            classId: c.className,
                            studentsToUpdate: [], // Need to push empty array to trigger Cleanup of OLD records
                        });
                    }
                });
            });

            console.log("2. 🚀 Payload gửi lên Server:", recordsToImport);

            const result = await processImportedAttendance(recordsToImport);

            if (result.success) {
                console.log("3. ✅ Đã Ghi Thành Công lên Server:", result.stats);
                toast.success(result.message || "Đã lưu điểm danh thành công!");
                onSuccess?.();
                onOpenChange(false);
            } else {
                console.error("3. ❌ Lỗi từ Server:", result.message);
    ...
    const resolveStudent = (recordIdx: number, classIdx: number, studentId: string, dbStudentCode: string) => {
        setProcessedData(prev => {
            const next = [...prev];
            const cls = next[recordIdx].classes[classIdx];
            const student = cls.matchedStudents.find(s => s.id === studentId);
            if (!student) return prev;

            const dbStudent = cls.allClassStudents.find(ds => ds.code === dbStudentCode);
            if (!dbStudent) return prev;

            const originalName = student.originalNameStr;

            // 1. Resolve the target student
            student.studentId = dbStudent.code;
            student.studentName = dbStudent.fullName;
            student.isMatched = true;

            // 2. Auto-resolve others with the same original name across all records
            next.forEach(r => {
                r.classes.forEach(c => {
                    c.matchedStudents.forEach(ms => {
                        if (!ms.isMatched && ms.originalNameStr === originalName) {
                            // Find the match in the local class pool
                            const localMatch = c.allClassStudents.find(ds => ds.code === dbStudentCode);
                            if (localMatch) {
                                ms.studentId = localMatch.code;
                                ms.studentName = localMatch.fullName;
                                ms.isMatched = true;
                            }
                        }
                    });
